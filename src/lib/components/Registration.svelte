...
<div class="p-8 shadow-lg w-96 card bg-base-100">
    <form on:submit|preventDefault={handleRegister} class="mb-3 space-y-4">
        ...
        <div class="form-control">
            ...
            <input id="passwordConfirm" class="w-full max-w-xs input input-bordered" type="password" bind:value={passwordConfirm} placeholder="Confirm Password" required aria-required="true"/>
        </div>
        <button class="w-full mt-4 btn btn-secondary" type="submit">Register</button>
        {#if errorMessage}
            <p class="text-red-500">{errorMessage}</p>
